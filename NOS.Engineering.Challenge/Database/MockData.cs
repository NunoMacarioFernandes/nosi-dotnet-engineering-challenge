...
public class MockData : IMockData<Content>
{
    ...
    public IDictionary<Guid, Content> GenerateMocks()
    {
        ...
        for (int i = 0; i < 10; i++)
        {
            ...
            var newMovie = new Content(id, title, subTitle, description, imageUrl, duration, startTime, endTime, genres);

            _cache.SetAsync(id, newMovie);

            mockContents.Add(id, newMovie);
        }
